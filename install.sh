...
then
    figlet $TEXT_WELCOME_UP \ $TEXT_WELCOME_DOWN
else 
...
#for PROOT users
if { [ $DEVICE_OS == 'GNU/Linux' ] && ! [ -e "/usr/bin/sudo" ]; }
then
    if ! [ $UID == "0" ] 
    then
...
ask_install_nodejs() {
    ...
    then
        ...
        then
            ...
            echo 'Extract NodeJS...'
            mkdir $TMPDIR/nodejs
            tar xfvz $TMPDIR/nodejs.tar.gz -C $TMPDIR/nodejs
            echo "Move File To $PREFIX (access super user require)..."
            sudo cp -rf $TMPDIR/nodejs/*/* $PREFIX
