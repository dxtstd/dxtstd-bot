...
const getImage = async function (keyword="sucrose") {
    return new Promise((resolve, reject) => {
        utils.scrapper.pinterest
        .keyword(keyword)
        .then(({ result }) => {
            const pin = result[Math.floor(Math.random() * result.length)]

           utils.fetcher.getBuffer(pin.image.best)
...
const createWelcome = async function ({background_image, pp_image}) {
    ...
    gradientBg.addColorStop(0, "rgba(0, 120, 255, 1)")
    gradientBg.addColorStop(1, "rgba(0, 200, 255, 1)")
    ...
    let encoder = HME.createH264MP4Encoder().then(encoder => {
        ...
        fs.writeFile("./result.mp4", Buffer.from(datax), () => {
          console.log("done!")
        })
    }).catch(console.error)
}

getImage("anime background")
.then(image => loadImage(image))
.then(background_image => {
    return utils.fetcher.getBuffer("https://pps.whatsapp.net/v/t61.24694-24/414709478_265079949730633_5497136441455526937_n.jpg?ccb=11-4&oh=01_Q5AaILopqI-0vU8kak4bLa2KHkeKrqiSiEeOkthguF8-MzZ9&oe=664DF6D3&_nc_sid=e6ed6c&_nc_cat=105")
    .then(image => loadImage(image))
    .then(pp_image => {
