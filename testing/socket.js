...
cai.sock.on("message", (msg) => {
    ...
    if (typeof msg == 'string') {
        ...
        if (jsong?.command == "update_turn" && jsong?.turn.candidates[0].is_final) {
            cai.chat.history.push({ name: jsong?.turn.author.name, message: jsong?.turn.candidates[0].raw_content })
            bot.sock.sendMessage(m.from, { text: jsong?.turn.author.name + ": " + jsong?.turn.candidates[0].raw_content })
        }
    }
})
