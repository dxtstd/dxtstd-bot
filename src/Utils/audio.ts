...
const EffectFFMPEG = {
    ...
    clear: [
        "equalizer=f=80:width_type=o:width=1:g=0.15", 
        "equalizer=f=160:width_type=o:width=1:g=0.2", 
        "equalizer=f=375:width_type=o:width=1:g=0.3", 
        "equalizer=f=750:width_type=o:width=1:g=-4", 
        "equalizer=f=1500:width_type=o:width=1:g=-3", 
        ...
        "equalizer=f=12000:width_type=o:width=4:g=5"
        ],
...
const processor = async function (input: any, opts: any): Promise < any > {
    const result = await new Promise((resolve, reject) => {
        ...
        let ffmpegCodeExit: number|string|undefined|null = 0;

        const sox = spawn('sox', opts.sox.args.result())
        let DataSoxStdErr = Buffer.from([])
        let soxClose: any;
        let soxCodeExit: number|string|undefined|null = 0;
        ...
        const exitHandler = function () {
            if ((ffmpegClose && soxClose) && (ffmpegCodeExit != 0 && soxCodeExit != 0)) {
                const error = `\n\tFFMPEG (exit code: ${ffmpegCodeExit}): ${String(DataFFMpegStdErr)}\n\tSOX (exit code: ${soxCodeExit}): ${String(DataSoxStdErr)}`
                reject(new Error(error))
            } else if ((ffmpegClose && soxClose) && (ffmpegCodeExit == 0 && soxCodeExit != 0)) {
                const error = `\n\tFFMPEG (exit code: ${ffmpegCodeExit}): ${String(DataFFMpegStdErr)}\n\tSOX (exit code: ${soxCodeExit}): ${String(DataSoxStdErr)}`
                reject(new Error(error))
            }
