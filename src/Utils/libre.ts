...
const convert = async function (Input: any, Ext: string='', opts: object={}) {
    try {
        const DEF_STDIO = 'inherit' //['pipe', 'pipe', 'pipe', 'ipc']
        const DEF_OPTS = {
            env: process.env,
        ...
        const ChangeExtOutput = function (input, output) {
            ...
            const  ExtInput = ((SplitInput)[0]).replace(SplitInput[0], output)
        ...
        switch (Ext) {
            case 'docx':
                Output = ChangeExtOutput(Input, 'docx')
                break;
            case 'pdf':
                Output = ChangeExtOutput(Input, 'pdf')
                break;
        }
