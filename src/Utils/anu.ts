...
const { Canvas, Image, loadImage, registerFont } = require("canvas")
const GIFEncoder = require('gifencoder')
const fs = require("fs")
const path = require("path")
registerFont(path.join(__dirname, '../../../', 'assets', 'font', 'Montserrat', 'Montserrat Bold Italic 700.ttf'), {
...
const createCanvas = async function ({background_image, pp_image}) {
    ...
    text.font = "50px \"Montserrat Regular\"";
    ...
    const encoder = new GIFEncoder(cmw, cmh)
    encoder.setRepeat(0)
    encoder.setDelay(1)
    ...
    for (let fireFile of assetsFire) {
        ...
        encoder.addFrame(gctx)
    }
    
    encoder.finish()
    void m.reply({ image: cm.toBuffer('image/png') })
}
